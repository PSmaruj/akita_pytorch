...
def main(target_idx: int, data_split: int, organism: str, data_name: str):
    ...
    # Access TensorFlow Dense layer parameters (weights and biases)
    tf_kernel = h5_file["model_weights/dense/dense"]['kernel:0'][:,target_idx]  # Weights (input_units, output_units)
    tf_kernel = tf_kernel[None, :] # so the shape is kept (1,80)
    tf_bias = h5_file["model_weights/dense/dense"]['bias:0'][target_idx]      # Bias (output_units,)
