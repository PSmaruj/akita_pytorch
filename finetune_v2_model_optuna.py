...
# =====================
def objective(trial, args, train_loader, val_loader, device, model_path):
    lr = trial.suggest_loguniform('lr', 1e-5, 1e-2)
    weight_clip = trial.suggest_float('weight_clip', 5.0, 25.0)
    l2_scale = trial.suggest_loguniform('l2_scale', 1e-6, 1e-3)
    early_stop = trial.suggest_int('early_stop_patience', 10, 40)
    ...
    print(f"\nStarting trial {trial.number} with lr={lr:.5g}, weight_clip={weight_clip}, l2_scale={l2_scale:.5g}, early_stop={early_stop}")
...
def main():
    ...
    study.optimize(lambda trial: objective(trial, args, train_loader, valid_loader, device, model_path), n_trials=args.n_trials)

    df = study.trials_dataframe()
    df.to_csv(f"/scratch1/smaruj/Akita_pytorch_models/finetuned/{args.organism}_models/{args.data_name}/losses/optuna_{args.data_name}_model{args.data_split}_all_trials.csv", index=False)
    ...
    # Save as CSV
    pd.DataFrame([best_info]).to_csv(f"/scratch1/smaruj/Akita_pytorch_models/finetuned/{args.organism}_models/{args.data_name}/losses/optuna_{args.data_name}_model{args.data_split}_best_trial.csv", index=False)
