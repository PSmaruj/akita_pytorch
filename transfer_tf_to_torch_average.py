...
def main(data_split: int, organism: str):
    ...
    # Final dense layer (target-dependent)
    print("Assigning target-specific dense layer...")

    pytorch_dense_layer = model.final.dense

    # Access TensorFlow Dense layer parameters (weights and biases)
    # DIFFERENCE -> average over all targets
    tf_kernel_all = h5_file["model_weights/dense_1/dense_1"]['kernel:0'][:]  # Weights (input_units, output_units)
    tf_bias_all = h5_file["model_weights/dense_1/dense_1"]['bias:0'][:]      # Bias (output_units,)

    # Average over all output units (targets)
    tf_kernel = tf_kernel_all.mean(1, keepdims=True).T  # → (1, input_units)
    tf_bias = tf_bias_all.mean()                           # → scalar
