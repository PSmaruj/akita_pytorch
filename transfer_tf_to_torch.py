...
Usage:
    python transfer_tf_to_torch.py --target_idx 0 --data_split 0 --organism mouse
"""
import argparse
import os
import torch
...
def main(target_idx: int, data_split: int, organism: str):
    ...
    # Paths
    data_name = "Krietenstein2019_H1hESC"
    ...
    # Access TensorFlow Dense layer parameters (weights and biases)
    tf_kernel = h5_file["model_weights/dense_1/dense_1"]['kernel:0'][:,target_idx]  # Weights (input_units, output_units)
    tf_kernel = tf_kernel[None, :] # so the shape is kept (1,80)
    tf_bias = h5_file["model_weights/dense_1/dense_1"]['bias:0'][target_idx]      # Bias (output_units,)
